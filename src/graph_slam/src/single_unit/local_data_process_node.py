        ...
        while not rospy.is_shutdown():
            ...
            if car.lidar.landmark is not None: 
                ...
                if len(Node_set) >= 2:
                    ...
                    """
                    pose logic:
                        if |d_odom - d_icp| >> 0 : use odom
                        else : use icp
                    =============================================

                    _, _, pose_icp = ScanMatching().icp(A[5], B[5])
                    pose_odom =  [ B[1] - A[1] , B[2] - A[2] ]
                    pose_error = ( (pose_icp[1] - pose_odom[0])**2 + (pose_icp[1] - pose_odom[0])**2 )**0.5
                    if abs(pose_error) > 0.3:
                        yaw_A = A[3] 
                        x_A , y_B = A[1], A[2]
                        T_A = v2t(np.array([yaw_A, x_A, y_A]))

                        yaw_B = B[3] 
                        x_B , y_B = B[1], B[2]
                        T_B = v2t(np.array([yaw_B, x_B, y_B]))
                        ...
                        v = np.linalg.inv(T_B).dot(dv)
                        x_BA = v[0,0]
                        y_BA = v[1,0]

                        T_BA = tf(np.array([yaw_BA, x_BA, y_BA]))
                        pose = t2v(T_BA)
                    """
